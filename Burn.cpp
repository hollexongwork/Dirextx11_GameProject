...
void Burn::Init()
{
	...
	Renderer::CreatePixelShader(&m_PixelShader, "shader/cso/EmissiveParticle.cso");

	// Load Texture
	m_Texture = Texture::Load("asset/texture/T_fire_explosion_pack_8x8_1024.png");
	m_ColorMap = Texture::Load("asset/texture/T_fire_gradient.png",true);
	...
	bufferDesc.ByteWidth = sizeof(Burn::Material);
	...
	// Particle Parameters
	m_LifeDist = std::uniform_real_distribution<float>(3.0f, 6.0f);
	m_SizeDist = std::uniform_real_distribution<float>(1.0f, 1.5f);
	m_RotaDist = std::uniform_real_distribution<float>(-30.0f, 30.0f);
	m_RotaSpeedDist = std::uniform_real_distribution<float>(-5.0f, 5.0f);
	m_VelocityDistX = std::uniform_real_distribution<float>(0.0f, 0.0f);
	m_VelocityDistY = std::uniform_real_distribution<float>(1.0f, 3.5f);
	m_VelocityDistZ = std::uniform_real_distribution<float>(0.0f, 0.0f);
	m_EmitPosDistX = std::uniform_real_distribution<float>(-0.85f, 0.85f);
	m_EmitPosDistY = std::uniform_real_distribution<float>(0.0f, 0.25f);
	m_EmitPosDistZ = std::uniform_real_distribution<float>(-1.5f, 1.8f);
...
void Burn::Update()
{
	...
	for (int i = 0; i < PARTICLE_MAX; i++)
	{
		if (m_Particle[i].Enable == true)
		{
			...
			//SubUV
			m_Material[i].Frame = static_cast<int>(life * 64.0f); // 8x8
			...
			t = Clamp(t, 0.0f, 1.0f);
			//m_Material[i].Color.w = Lerp(0.25f, 0.0f, t);
...
void Burn::Draw()
{
	...
	Renderer::GetDeviceContext()->PSSetShaderResources(1, 1, &m_ColorMap);
	...
	for (int i = 0; i < PARTICLE_MAX; i++)
	{
		if (!m_Particle[i].Enable)
		{
...

			{
				...
				position = Position;
				position += m_AxisX * m_EmitPosDistX(engine);
				position += m_AxisY * m_EmitPosDistY(engine);
				position += m_AxisZ * m_EmitPosDistZ(engine);
