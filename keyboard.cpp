...
static Keyboard_State old_state = {};

static Keyboard_State trigger_state = {};
...
{
    const Keyboard_State* now_state = &gState;
   
    LPBYTE pt = (LPBYTE)&trigger_state;
    LPBYTE pn = (LPBYTE)now_state;
    LPBYTE po = (LPBYTE)&old_state;
    for (int i = 0; i < sizeof(Keyboard_State); i++)
    {
        pt[i] = (po[i] ^ pn[i]) & pn[i];  //keyDown
        //pt[i] = (po[i] ^ pn[i]);      //keyUp&keyDown
        //pt[i] = (po[i] ^ pn[i])&po[i];//keyUp
...
{
    return Keyboard_IsKeyDown(key, &trigger_state);
}
/*---------------------------------------------------------------*/
