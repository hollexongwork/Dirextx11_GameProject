...
void DamageSmoke::Init()
{
	...
	bufferDesc.ByteWidth = sizeof(DamageSmoke::Material);
	...
	// Particle Parameters
	m_LifeDist = std::uniform_real_distribution<float>(3.0f, 5.5f);
	m_SizeDist = std::uniform_real_distribution<float>(0.75f, 1.25f);
	...
	m_VelocityDistY = std::uniform_real_distribution<float>(0.75f, 1.75f);
	m_VelocityDistZ = std::uniform_real_distribution<float>(0.0f, 0.0f);
	m_EmitPosDistX = std::uniform_real_distribution<float>(-0.65f, 0.65f);
	m_EmitPosDistY = std::uniform_real_distribution<float>(0.0f, 0.25f);
	m_EmitPosDistZ = std::uniform_real_distribution<float>(-2.0f, 1.5f);

	m_Drag = 0.5f;
}

void DamageSmoke::Uninit()
{
	Particle::Uninit();
...
void DamageSmoke::EmissionParticle()
{
	for (int i = 0; i < PARTICLE_MAX; i++)
	{
